...
def download_invoice_excel(request):
    ...
    header_row = ["ID", "Full Name", "Address", "GST no.", "Invoice number", "Course" , "Particular detail", "Amount" , "Discount" , "Total amount" , "CGST %" , "SGST %" , "CGST amount" ,"SGST amount" , "Total amount payable"]
    ...
    for inv in invoice_all:
        data_row = [f"INV-{inv.id}", inv.full_name_invoice, inv.address_invoice, inv.GST_invoice , inv.invoice_number , str(inv.coaching_type_course_invoice), inv.particular_invoice, inv.amount_invoice,
                    inv.any_discount_invoice , inv.total_amount_invoice , inv.cgst , inv.sgst , inv.cgst_amount , inv.sgst_amount, inv.total_amount_payable_invoice ]
